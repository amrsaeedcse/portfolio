...
export default function Work({ onProjectClick }) {
  ...
  return (
    <section id="work" className="relative">
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          📱 MOBILE LAYOUT (< md): Natural Vertical Flow + Touch Horizontal Swipe
          ══════════════════════════════════════════════════════════════════════ */}
      <div className="block md:hidden py-10 px-3 sm:px-6">

        {/* Section Header */}
        <div className="mb-5 pb-3 border-b border-current/15">
          <div className="flex items-center justify-between gap-2 mb-1">
            <span className="bp-stamp text-[#FF4400] border-[#FF4400] !text-[0.6rem] !py-0.5 !px-2">
              SHEET 03 // DRAWINGS
            </span>
            <span className="font-mono text-[0.62rem] text-[#3A57C4] font-bold tracking-wider">
              SWIPE HORIZONTALLY ⇄
            </span>
          </div>
          ...
          <p className="font-mono text-[0.68rem] text-inherit/70 mt-1">
            05 FEATURED BLUEPRINTS · SWIPE TO INSPECT
          </p>
        </div>

        {/* Native Touch-Scrollable Horizontal Carousel */}
        <div className="overflow-x-auto snap-x snap-mandatory flex gap-4 pb-4 pt-1 -mx-3 px-3 scrollbar-none touch-pan-x will-change-scroll">
          {featured.map((proj, idx) => (
            <div
              key={proj.id}
              className="w-[88vw] max-w-[360px] h-[520px] flex-none snap-center"
            >
              <div className="h-full sheet-frame overflow-hidden shadow-lg flex flex-col justify-between border border-current/20 bg-inherit rounded-sm">
                ...
                <div className="flex items-center justify-between px-3.5 py-2 border-b border-current/15 bg-current/5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-[0.65rem] text-inherit/70">
                      0{idx + 1} OF 05
                    </span>
                  </div>
                ...
                <div className="p-3.5 flex-1 flex flex-col justify-between overflow-hidden">
                  <div>
                    ...
                    <p className="font-mono text-[0.7rem] text-[#FF4400] font-semibold mt-0.5 line-clamp-1">
                      {proj.subtitle}
                    </p>
                    ...
                    <div
                      ...
                      className="my-2.5 relative h-36 w-full overflow-hidden border border-current/20 bg-current/10 cursor-pointer shadow-inner group rounded-sm"
                    >
                      <img
                        src={proj.img}
                        alt={proj.title}
                    ...
                    {/* Tech Pills */}
                    <div className="flex flex-wrap gap-1 mt-2 max-h-[46px] overflow-hidden">
                  ...
                  {/* Action CTAs */}
                  <div className="grid grid-cols-2 gap-2 pt-2.5 border-t border-current/15 mt-2">
                    <button
                      onClick={() => { playSwitchClick(); onProjectClick(proj); }}
          ...
          <div className="w-[82vw] max-w-[340px] h-[520px] flex-none snap-center">
        ...
        {/* Mobile Swipe Hint Dots Indicator */}
        <div className="flex items-center justify-center gap-1.5 mt-2">
          {featured.map((_, i) => (
            <div key={i} className="w-1.5 h-1.5 rounded-full bg-current/30" />
          ))}
