...
export default function Work({ onProjectClick }) {
  ...
  // Native Mobile Touch Gestures: Horizontal finger swiping translates the cards smoothly
  useEffect(() => {
    ...
    const onTouchMove = (e) => {
      ...
      // Lock horizontal gesture if horizontal movement dominates
      if (!isHorizontal && Math.abs(dx) > 6 && Math.abs(dx) > Math.abs(dy) * 1.1) {
        isHorizontal = true;
      }

      if (isHorizontal) {
        // Prevent default vertical jumping while swiping cards horizontally
        if (e.cancelable) {
          e.preventDefault();
        }
        const speedMultiplier = 2.2;
        window.scrollBy({ top: -dx * speedMultiplier, behavior: 'auto' });
        startX = currentX;
        startY = currentY;
      }
    };
  ...
  return (
    <section id="work" ref={targetRef} className="relative h-[460vh]">
      ...
      <div
        ref={containerRef}
        className="sticky top-[3vh] h-screen w-full overflow-hidden flex flex-col justify-center px-3 sm:px-8 md:px-14 touch-pan-y"
      >
