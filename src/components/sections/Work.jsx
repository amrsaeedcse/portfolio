...
export default function Work({ onProjectClick }) {
  ...
  // Handle mobile horizontal scroll index
  ...
  return (
    <>
      {/* ══════════════════════════════════════════════════════════════════════
          MOBILE / TABLET VIEW: Native Horizontal Touch Swipe Carousel
          ══════════════════════════════════════════════════════════════════════ */}
      <section id="work" className="block md:hidden py-10 px-3 bg-inherit border-b border-current/15">
        
        {/* Mobile Header */}
        <div className="mb-4 px-1 flex flex-col gap-1.5 select-none border-b border-current/15 pb-3">
        ...
        {/* Native Touch-Scrollable Horizontal Reel */}
        <div
          ref={mobileScrollRef}
          onScroll={handleMobileScroll}
          className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-4 pt-1 px-1 no-scrollbar scroll-smooth -mx-3 px-3 will-change-scroll"
          style={{ WebkitOverflowScrolling: 'touch' }}
        >
          {featured.map((proj, idx) => (
            <div
              key={proj.id}
              className="w-[86vw] max-w-[370px] snap-center flex-none"
            >
              <div className="h-full sheet-frame overflow-hidden shadow-lg flex flex-col justify-between border border-current/20 bg-inherit rounded-sm">
                ...
                <div className="p-4 flex flex-col justify-between flex-1">
                  <div>
                    ...
                    {/* Tech Badges */}
                    <div className="flex flex-wrap gap-1.5 mt-3 max-h-[56px] overflow-hidden">
          ...
          <div className="w-[86vw] max-w-[370px] snap-center flex-none">
            <div className="h-full sheet-frame p-6 text-center shadow-lg flex flex-col items-center justify-center border border-current/20 bg-inherit rounded-sm min-h-[460px]">
              ...
              <button
                ...
                className="bp-btn-primary mt-6 !py-3 !px-6 min-h-[44px] w-full"
              >
        ...
        {/* Mobile Reel Dots Indicator */}
        <div className="flex items-center justify-center gap-1.5 mt-2">
          {Array.from({ length: featured.length + 1 }).map((_, i) => (
            <div
              key={i}
              className={`h-1.5 rounded-full transition-all duration-300 ${
                mobileActiveIdx === i ? 'w-6 bg-[#FF4400]' : 'w-1.5 bg-current/25'
              }`}
            />
          ))}
        </div>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          ...
          ══════════════════════════════════════════════════════════════════════ */}
      <section id="work" ref={targetRef} className="hidden md:block relative h-[460vh]">
