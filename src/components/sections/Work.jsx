...
export default function Work({ onProjectClick }) {
  ...
  // Smooth horizontal translation across devices along the exact same path
  ...
  // Touch gesture handling: Pin screen vertically and advance horizontally with horizontal finger swiping
  useEffect(() => {
    ...
    const onTouchMove = (e) => {
      if (e.touches.length !== 1 || !startX || !targetRef.current) return;
      const currentX = e.touches[0].clientX;
      const currentY = e.touches[0].clientY;
      const dx = currentX - startX;
      const dy = currentY - startY;

      // When horizontal finger swipe is detected:
      if (Math.abs(dx) > Math.abs(dy)) {
        const rect = targetRef.current.getBoundingClientRect();
        const maxScroll = targetRef.current.offsetHeight - window.innerHeight;
        const currentScrollY = -rect.top;
        const currentProgress = Math.min(1, Math.max(0, currentScrollY / maxScroll));

        // If we are within the pinned cards range (0 to 1), lock vertical scrolling and translate horizontally
        const isSwipingForward = dx < 0 && currentProgress < 0.99;
        const isSwipingBackward = dx > 0 && currentProgress > 0.01;

        if (isSwipingForward || isSwipingBackward) {
          if (e.cancelable) {
            e.preventDefault(); // Lock vertical scroll
          }
          const sensitivity = 2.8;
          window.scrollBy({ top: -dx * sensitivity, behavior: 'auto' });
          startX = currentX;
          startY = currentY;
        }
      }
  ...
  return (
    <section id="work" ref={targetRef} className="relative h-[460vh]">
      ...
      {/* ── Sticky Pinned Viewport Container (Active on Both Mobile & Desktop) ─ */}
      <div
        ref={containerRef}
        className="sticky top-[3vh] h-screen w-full overflow-hidden flex flex-col justify-center px-3 sm:px-8 md:px-14"
      >
              ...
              <TiltCard maxTilt={4} className="h-full w-full">
                <div className="h-full sheet-frame p-6 sm:p-10 text-center shadow-xl flex flex-col items-center justify-center border border-current/20 bg-inherit">
                  ...
                  <button
                    ...
                    onMouseEnter={playHoverTick}
                    className="bp-btn-primary mt-6 sm:mt-8 !py-3 !px-8 min-h-[46px] w-full sm:w-auto font-bold"
                  >
